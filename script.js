...
const QUIZ_DATA = {
  ...
  'get-endpoints': [
    ...
    {
      q: 'When GET /items/999 is called and the item does not exist, the client typically receives:',
      options: ['404 with {"detail": "Item not found"}', '200 with an empty array', '201 Created with a new item', '500 with no JSON body'],
      correctIndex: 0,
      explanation: 'HTTPException(404) returns a JSON object with a "detail" field describing the error — the client can read response.status and response.json().',
    },
    {
    ...
    {
      q: 'GET /items/abc (where item_id must be an int) returns to the client:',
      options: ['200 OK with the item', '404 Not Found', '422 Unprocessable Entity with validation errors', '201 Created'],
      correctIndex: 2,
      explanation: 'FastAPI validates path types before your function runs. "abc" cannot parse as int, so the client gets 422 with a detail array explaining the problem.',
    },
    {
      q: 'A successful GET /items/1 response body looks like:',
      options: ['A JSON array of all items', 'A single JSON object with id, name, and price', 'An empty string only', 'An HTML login page'],
      correctIndex: 1,
      explanation: 'Fetching one resource returns one JSON object. Listing many items (GET /items) returns a JSON array.',
    },
    {
      q: 'In JavaScript fetch(), why check res.status before res.json()?',
      options: ['JSON only works on status 500', 'Error responses (404, 422) also return JSON that must be handled differently', 'res.json() is deprecated', 'GET requests never return JSON'],
      correctIndex: 1,
      explanation: 'FastAPI returns JSON for both success and many errors. The client must branch on status — e.g. show data on 200, show detail on 404/422.',
    },
    {
      q: 'FastAPI 422 error responses use a "detail" field that is often:',
      options: ['Always a single string only', 'An array of objects with loc, msg, and type', 'A base64 image', 'Empty on every request'],
      correctIndex: 1,
      explanation: 'Validation errors list each problem: "loc" shows where (path/body/query), "msg" explains what went wrong, "type" categorizes the error.',
    },
    {
  ...
  'post-endpoints': [
    ...
    {
      q: 'After a successful POST /items, the client receives status 201 and a body containing:',
      options: ['Only {"ok": true}', 'The created item including server-assigned id', 'An empty body always', 'A redirect URL only'],
      correctIndex: 1,
      explanation: '201 Created responses include the new resource JSON so the client can confirm fields and use the new id immediately.',
    },
    {
      q: 'POST with body {"name": "Keyboard"} but missing price returns to the client:',
      options: ['200 OK with the item', '201 Created anyway', '422 with detail listing the missing "price" field', '404 Not Found'],
      correctIndex: 2,
      explanation: 'Pydantic requires all fields on ItemCreate. Missing price triggers 422 with loc ["body", "price"] and msg "Field required".',
    },
    {
    ...
    {
      q: 'POST with {"name": "Keyboard", "price": "free"} (string instead of number) returns:',
      options: ['201 with price stored as "free"', '200 OK', '500 Server Error with no body', '422 with a float_parsing validation error on price'],
      correctIndex: 3,
      explanation: 'price: float rejects non-numeric strings. The client gets 422 and a detail entry pointing at body.price.',
    },
    {
      q: 'In a 422 response, "loc": ["body", "price"] tells the client:',
      options: ['The error is in the request JSON body, field price', 'The server is located in Paris', 'The price field was updated successfully', 'The error is in the URL path only'],
      correctIndex: 0,
      explanation: '"loc" is location: ["body", "price"] means the problem is in the JSON body at the price key. Path errors use ["path", "item_id"].',
    },
    {
      q: 'POST requests typically send data in:',
      options: ['The JSON request body', 'The URL only, never a body', 'Response headers only', 'The CSS file'],
      correctIndex: 0,
      explanation: 'POST carries resource data in the body with Content-Type: application/json. The client serializes an object to JSON before sending.',
    },
    {
      q: 'Using curl -i when calling POST helps the client developer see:',
      options: ['Only the Python source code', 'HTTP status line, headers, and response body together', 'Database passwords', 'Git commit history'],
      correctIndex: 1,
      explanation: '-i includes response headers (e.g. HTTP/1.1 201 Created) plus the JSON body — useful for debugging what the API actually returned.',
    },
  ],
  ...
  'edit-delete-endpoints': [
    ...
    {
      q: 'DELETE /items/1 with status_code=204 means the client receives:',
      options: ['200 with the deleted item JSON', '204 success with an empty response body', '422 validation error', '201 Created'],
      correctIndex: 1,
      explanation: '204 No Content signals success with nothing to parse — clients should check status, not assume JSON exists.',
    },
    {
    ...
    {
      q: 'Successful PATCH /items/1 with {"price": 899.99} returns to the client:',
      options: ['Only {"price": 899.99}', '204 with no body', '200 with the full updated item (id, name, price)', '404 always'],
      correctIndex: 2,
      explanation: 'PATCH merges changes and returns the complete resource so the client sees the final state without another GET.',
    },
    {
      q: 'PATCH /items/42 when id 42 does not exist returns:',
      options: ['200 with null', '201 Created', '500 Internal Server Error only', '404 with {"detail": "Item not found"}'],
      correctIndex: 3,
      explanation: 'find_item raises HTTPException(404) — same JSON error shape the client sees on GET /items/999.',
    },
    {
      q: 'After DELETE succeeds (204), calling GET /items/1 should return:',
      options: ['404 Not Found', '200 with the old item', '201 Created', '422 Unprocessable Entity'],
      correctIndex: 0,
      explanation: 'The resource is gone. The client gets 404 on subsequent reads — a sign the delete worked.',
    },
    {
      q: 'PATCH with {"price": "cheap"} (invalid type) returns to the client:',
      options: ['200 with price "cheap"', '422 with validation errors on body.price', '204 No Content', '401 Unauthorized'],
      correctIndex: 1,
      explanation: 'Even on PATCH, Pydantic validates types. Invalid price strings produce 422 before the update logic runs.',
    },
    {
